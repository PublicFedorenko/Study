...
{
    public class Jake : IFormat
    {
        /*
         * Fields for dissassembling
         */
        private readonly string _regExpression = @"(?x)\s* <" + TypeParts.DataType + @"> \s* 
                                              (<(?<keys>\w+)> \s* (?<values>(\w|\.)+) \s* </(\w+)> \s*)+ 
                                              </\w+>\s*";
        /*
        ...
        private readonly string _keyValuePartWithSplitter = "  <" + TypeParts.Key + ">" + TypeParts.Value + "</" + TypeParts.Key + ">\n";
        private readonly string _keyValuePart = "  <" + TypeParts.Key + ">" + TypeParts.Value + "</" + TypeParts.Key + ">\n";
        private readonly string _closingPart = "</" + TypeParts.DataType + ">\n";
        ...
        public string[][,] Dissassemble(string data, Type objType)
        {
            ...
            Regex objRegex = new Regex(_regExpression.Replace(TypeParts.DataType.ToString(), typeName));  // add name of class to RegExpression
        ...
        public string Assemble<T>(T[] obj)
        {
            ...
            for (int i = 0; i < obj.Length; i++)  // for each object
            {
                ...
                if (definedProperties.GetLength(0) > 1)
                {
                    ...
                    /* Closing part */
                    formatedData += _closingPart.Replace(TypeParts.DataType.ToString(), typeof(T).Name);
                }
